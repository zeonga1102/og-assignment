...
class StatisticsView(APIView):
    ...
    def get(self, request):
        ...
        """
        query = Q(status_id=1) & (Q(work__size__lte=100) | Q(work__size__isnull=True))
        artist_data = Artist.objects.annotate(avg_price=Avg(F("work__price"))).prefetch_related("work_set").prefetch_related("exhibition_set").filter(query)
        
        for ad in artist_data:
            setattr(ad, "number_of_lte_100", ad.work_set.all().count())
            if ad.avg_price is None:
                ad.avg_price = 0
            setattr(ad, "number_of_exhibition", ad.exhibition_set.all().count())
